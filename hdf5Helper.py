...
def writeH5(pressure,u,v,w,velmag,filename):
  ...
  velo_group.create_dataset("velmag",data=velmag)

  # Store velocity data into the velo_group of h5 file
  pres_group = f.create_group("pres_group")
  pres_group.create_dataset("presmag",data=pressure)

  f.close()
  
def writeH5_GOL(states,filename):
    """
    ...
    Parameters
    ----------
    states : NxN array containing Boolean state (alive/dead).
    filename : string
    ...
    state_group.create_dataset("state",data=states)
...
def writeXdmf_GOL(dims,dx,filename,h5_file):
    f = open(filename,'w')
    f.write('<?xml version="1.0" ?>\n')
    f.write('<!DOCTYPE Xdmf SYSTEM "Xdmf.dtd" []>\n')
    f.write('<Xdmf xmlns:xi="http://www.w3.org/2003/XInclude" Version="2.1">\n')
